		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			<el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				...
				<el-form-item label="媒体属性" prop="brandId">
					<el-upload
							class="upload-demo"
							action="http://localhost:2000/services/common/fastdfs"
	...
	export default {
		...
		methods: {
            ...
            handleSuccessImg(response, file, fileList){
                let fileId = response.object;//从响应中获取到fileId
		...
		watch:{
		    'mediasList':function(newData){
				...
				for(var i=0;i<newData.length;i++){
                    newMediasIds.push(newData[i].response.object);
				}
			...
			'skuProperties':{
		        handler(newData){
					...
					let result = newSkuProperties.reduce((lastResult,currentEle,index)=>{ //lastResult [{}]
                        let newArr = []
                        lastResult.forEach(e1=>{ //e1  {}
							...
							for(var i=0;i<currentEle.options.length;i++){ //currentEle.options  ["萝莉","御姐"]
                                ...
                                //最后一次累加的时候再添加price和store属性
                                /*if(index == newSkuProperties.length-1) {
                                    Object.assign(obj,{price:0,store:0});
                                }*/
                                Object.assign(obj,{price:0,store:0});
